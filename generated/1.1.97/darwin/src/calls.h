...
Napi::Value _vkCreateMacOSSurfaceMVK(const Napi::CallbackInfo& info) {
  ...
  _VkMacOSSurfaceCreateInfoMVK* obj1;
  VkMacOSSurfaceCreateInfoMVK *$p1 = nullptr;
  if (info[1].IsObject()) {
    Napi::Object obj = info[1].As<Napi::Object>();
    if (!(obj.InstanceOf(_VkMacOSSurfaceCreateInfoMVK::constructor.Value()))) {
      NapiObjectTypeError(info[1], "argument 2", "[object VkMacOSSurfaceCreateInfoMVK]");
      return env.Undefined();
    }
    obj1 = Napi::ObjectWrap<_VkMacOSSurfaceCreateInfoMVK>::Unwrap(obj);
    if (!obj1->flush()) return env.Undefined();
    $p1 = &obj1->instance;
  } else if (info[1].IsNull()) {
    $p1 = nullptr;
