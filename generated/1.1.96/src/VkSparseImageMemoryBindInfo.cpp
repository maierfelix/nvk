...
bool _VkSparseImageMemoryBindInfo::flush() {
  ...
  if (!(self->pBinds.IsEmpty())) {
    ...
    std::string msg = "Expected '[object VkSparseImageMemoryBind]' for 'VkSparseImageMemoryBindInfo.pBinds' but got '" + details + "'";
...
NAN_GETTER(_VkSparseImageMemoryBindInfo::Getimage) {
  ...
  if (!value->IsNull()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected '[object VkImage]' for 'VkSparseImageMemoryBindInfo.image' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
      return;
    }
...
NAN_GETTER(_VkSparseImageMemoryBindInfo::GetbindCount) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkSparseImageMemoryBindInfo.bindCount' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}// pBinds
