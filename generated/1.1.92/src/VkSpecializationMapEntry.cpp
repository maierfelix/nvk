...
NAN_GETTER(_VkSpecializationMapEntry::GetconstantID) {
  ...
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkSpecializationMapEntry.constantID' but got '" + details + "'";
...
NAN_GETTER(_VkSpecializationMapEntry::Getoffset) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkSpecializationMapEntry.offset' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}// size
NAN_GETTER(_VkSpecializationMapEntry::Getsize) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkSpecializationMapEntry.size' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}