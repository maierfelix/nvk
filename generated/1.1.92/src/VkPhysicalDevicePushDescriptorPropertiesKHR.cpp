...
void _VkPhysicalDevicePushDescriptorPropertiesKHR::Initialize(Nan::ADDON_REGISTER_FUNCTION_ARGS_TYPE target) {
  ...
  SetPrototypeAccessor(proto, Nan::New("maxPushDescriptors").ToLocalChecked(), GetmaxPushDescriptors, SetmaxPushDescriptors, ctor);
...
NAN_METHOD(_VkPhysicalDevicePushDescriptorPropertiesKHR::New) {
  if (info.IsConstructCall()) {
    ...
    if (info[0]->IsObject()) {
      ...
      v8::Local<v8::String> sAccess2 = Nan::New("maxPushDescriptors").ToLocalChecked();
...
}NAN_SETTER(_VkPhysicalDevicePushDescriptorPropertiesKHR::SetsType) {
  _VkPhysicalDevicePushDescriptorPropertiesKHR *self = Nan::ObjectWrap::Unwrap<_VkPhysicalDevicePushDescriptorPropertiesKHR>(info.This());
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkPhysicalDevicePushDescriptorPropertiesKHR.sType' but got '" + details + "'";
...
}NAN_SETTER(_VkPhysicalDevicePushDescriptorPropertiesKHR::SetmaxPushDescriptors) {
  _VkPhysicalDevicePushDescriptorPropertiesKHR *self = Nan::ObjectWrap::Unwrap<_VkPhysicalDevicePushDescriptorPropertiesKHR>(info.This());
  if (value->IsNumber()) {
    self->instance.maxPushDescriptors = static_cast<uint32_t>(Nan::To<int64_t>(value).FromMaybe(0));
  } else {
    
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkPhysicalDevicePushDescriptorPropertiesKHR.maxPushDescriptors' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}