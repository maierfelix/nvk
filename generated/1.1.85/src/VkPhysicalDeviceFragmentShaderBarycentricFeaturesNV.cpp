...
NAN_GETTER(_VkPhysicalDeviceFragmentShaderBarycentricFeaturesNV::GetsType) {
  ...
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkPhysicalDeviceFragmentShaderBarycentricFeaturesNV.sType' but got '" + details + "'";
...
NAN_GETTER(_VkPhysicalDeviceFragmentShaderBarycentricFeaturesNV::GetfragmentShaderBarycentric) {
  ...
  if (value->IsBoolean() || value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkPhysicalDeviceFragmentShaderBarycentricFeaturesNV.fragmentShaderBarycentric' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}