...
NAN_GETTER(_VkPhysicalDeviceExternalFenceInfo::GetsType) {
  ...
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkPhysicalDeviceExternalFenceInfo.sType' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}// pNext
NAN_GETTER(_VkPhysicalDeviceExternalFenceInfo::GetpNext) {
  _VkPhysicalDeviceExternalFenceInfo *self = Nan::ObjectWrap::Unwrap<_VkPhysicalDeviceExternalFenceInfo>(info.This());
}NAN_SETTER(_VkPhysicalDeviceExternalFenceInfo::SetpNext) {
  _VkPhysicalDeviceExternalFenceInfo *self = Nan::ObjectWrap::Unwrap<_VkPhysicalDeviceExternalFenceInfo>(info.This());
}// handleType
NAN_GETTER(_VkPhysicalDeviceExternalFenceInfo::GethandleType) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkPhysicalDeviceExternalFenceInfo.handleType' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}