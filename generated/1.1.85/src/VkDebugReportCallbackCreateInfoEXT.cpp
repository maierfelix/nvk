...
NAN_GETTER(_VkDebugReportCallbackCreateInfoEXT::GetsType) {
  ...
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkDebugReportCallbackCreateInfoEXT.sType' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}// pNext
NAN_GETTER(_VkDebugReportCallbackCreateInfoEXT::GetpNext) {
  _VkDebugReportCallbackCreateInfoEXT *self = Nan::ObjectWrap::Unwrap<_VkDebugReportCallbackCreateInfoEXT>(info.This());
}NAN_SETTER(_VkDebugReportCallbackCreateInfoEXT::SetpNext) {
  _VkDebugReportCallbackCreateInfoEXT *self = Nan::ObjectWrap::Unwrap<_VkDebugReportCallbackCreateInfoEXT>(info.This());
}// flags
NAN_GETTER(_VkDebugReportCallbackCreateInfoEXT::Getflags) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkDebugReportCallbackCreateInfoEXT.flags' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}