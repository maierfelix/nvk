...
NAN_GETTER(_VkClearDepthStencilValue::Getdepth) {
  ...
  if (value->IsNumber()) {
    ...
    std::string msg = "Expected 'Number' for 'VkClearDepthStencilValue.depth' but got '" + details + "'";
...
NAN_GETTER(_VkClearDepthStencilValue::Getstencil) {
  ...
  if (value->IsNumber()) {
    ...
    std::string details = getV8ObjectDetails(value);
    if (details[0] == '#') details = "[object " + (details.substr(2, details.length() - 2 - 1)) + "]";
    std::string msg = "Expected 'Number' for 'VkClearDepthStencilValue.stencil' but got '" + details + "'";
    Nan::ThrowTypeError(msg.c_str());
    return;
  }
}