...
let commandPool = new VkCommanPool();

function getShaderFile(path) {
  return new Promise(resolve => {
    fs.readFile(path, null, (err, res) => {
...
async function init() {
  ...
  const vertSrc = await getShaderFile("./test/basic-vert.spv");
  const fragSrc = await getShaderFile("./test/basic-frag.spv");
  ...
  if ((result = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, [graphicsPipelineInfo], null, [pipeline])) !== VK_SUCCESS) {
  ...
  if ((result = vkCreateCommandPool(device, commandPoolInfo, null, commandPool)) !== VK_SUCCESS) {
